...
const ProfileHeader = ({ player, stats2025 }: { player: Player, stats2025: PlayerStats | null }) => (
    <div className="bg-nmfc-navy rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl">
        {/* Background Gradient Accent */}
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-nmfc-royal/20 to-transparent"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="relative group">
                ...
                <div className="w-40 h-40 rounded-full border-4 border-yellow-400 p-1 bg-nmfc-navy shadow-lg relative z-10">
                    <img
                ...
                <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-yellow-400 text-nmfc-navy text-sm font-bold px-3 py-1 rounded-full shadow-md z-20">
                    #{player.jumper_no}
                </div>
            </div>
            ...
            <div className="text-center md:text-left flex-1">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-blue-200 text-sm font-medium uppercase tracking-wider mb-4">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-8 mt-6">
                    ...
                    <div className="text-center">
                        <div className="text-2xl font-bold text-yellow-400">{player.games}</div>
                ...
                {stats2025 && (
                    <div className="mt-8 pt-6 border-t border-white/10 flex flex-wrap justify-center md:justify-start gap-x-8 gap-y-4">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-blue-300 font-black uppercase tracking-widest mb-1">AF Avg</span>
                            <span className="text-xl font-black text-white">{stats2025.af_avg.toFixed(1)}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-blue-300 font-black uppercase tracking-widest mb-1">Disposals</span>
                            <span className="text-xl font-black text-white">{stats2025.disposals_avg.toFixed(1)}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-blue-300 font-black uppercase tracking-widest mb-1">Marks</span>
                            <span className="text-xl font-black text-white">{stats2025.marks_avg.toFixed(1)}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-blue-300 font-black uppercase tracking-widest mb-1">Tackles</span>
                            <span className="text-xl font-black text-white">{stats2025.tackles_avg.toFixed(1)}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-[10px] text-blue-300 font-black uppercase tracking-widest mb-1">Goals</span>
            ...
            <div className="hidden md:block text-right">
                <div className={`text-6xl font-black ${player.readiness && player.readiness.score > 8 ? 'text-green-400' : 'text-yellow-400'}`}>
                    {player.readiness?.score.toFixed(1)}
                </div>
                <div className="text-xs text-blue-300 uppercase tracking-widest">Readiness</div>
            </div>
        </div>
    </div>
);
...
const AttributeCard = ({ title, text }: { title: string; text?: string }) => (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full">
        <h3 className="text-yellow-600 text-xs font-bold uppercase tracking-widest mb-2">{title}</h3>
...
export const PlayerDetail = () => {
    ...
    // Moving average calculation for Match Ratings
    const chartMatchData = matchRatings.map((m, idx) => {
        const last3 = matchRatings.slice(Math.max(0, idx - 2), idx + 1);
        const avg3 = last3.reduce((sum, r) => sum + r.rating, 0) / last3.length;
        return {
            ...m,
    ...
    const compositeScore = idpData.length > 0
        ? (idpData.reduce((sum, d) => sum + d.Coach, 0) / idpData.length).toFixed(1)
        : '0.0';

    const matchAvg = matchRatings.length > 0
        ? (matchRatings.reduce((sum, r) => sum + r.rating, 0) / matchRatings.length).toFixed(1)
        : '0.0';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <Link to="/players" className="flex items-center gap-2 text-gray-500 hover:text-nmfc-royal transition-colors font-medium">
                    <ChevronLeft size={20} />
                    Back to Squad
                </Link>
                <div className="flex gap-2">
                    <button className="p-2 hover:bg-white rounded-xl text-gray-400 hover:text-blue-600 transition-colors"><Share2 size={18} /></button>
                    <button className="p-2 hover:bg-white rounded-xl text-gray-400 hover:text-blue-600 transition-colors"><Printer size={18} /></button>
                </div>
            </div>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                        <div className="flex-1">
                            <h3 className="text-xl font-black uppercase tracking-tight mb-6 flex items-center gap-3">
                                ...
                                <span className="text-[10px] text-blue-300 font-bold bg-white/5 px-3 py-1 rounded-full border border-white/10 uppercase tracking-widest ml-auto">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                <div className="space-y-1">
                                    <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Sleep</div>
                                    <div className="text-2xl font-black text-white">{latestWellbeing.sleep_score}/10</div>
                                </div>
                                <div className="space-y-1">
                                    <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Soreness</div>
                                    <div className="text-2xl font-black text-white">{latestWellbeing.soreness_score}/10</div>
                                </div>
                                <div className="space-y-1">
                                    <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest">Mood</div>
                                    <div className="text-2xl font-black text-white">{10 - latestWellbeing.stress_score}/10</div>
                                </div>
                                <div className="space-y-1">
                                    <div className="text-[10px] font-black text-yellow-400 uppercase tracking-widest">Readiness</div>
                            ...
                            {latestWellbeing.notes && (
                                <div className="mt-8 p-5 bg-white/5 rounded-2xl border border-white/10">
                                    <div className="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-2">Check-In Notes</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                    <h3 className="font-bold text-xl text-gray-900 border-b-2 border-yellow-400 pb-2 mb-6">
                ...
                {/* Match Ratings Chart */}
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                    <div className="flex items-center justify-between border-b-2 border-yellow-400 pb-2 mb-8">
